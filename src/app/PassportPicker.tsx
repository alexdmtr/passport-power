...
export default function PassportPicker({ requirements }: PassportPickerProps) {
  const passports = useMemo(() => [...new Set(requirements.map(req => req.Passport))], [requirements]);
  ...
  return (
    <div className="passport-picker">
      ...
      <select className="passport-select" value={selectedPassport} onChange={(e) => setSelectedPassport(e.target.value)}>
      ...
      <ul>
        {selectedPassport && requirements.filter(req => req.Passport === selectedPassport).map(country => <li key={country.Destination}>{country.Destination} - {country.Requirement}</li>)}
      </ul>
    </div>
  );
}